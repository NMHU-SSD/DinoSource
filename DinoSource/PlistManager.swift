...
let plistFileName:String = "dinosaurs"
...
class PlistManager {
  ...
  func startPlistManager() {
    ...
    func updateCaughtKeysWith(newDino:String) {
        caughtKeys.append(newDino)
        caughtKeys.sort()
    }
  ...
  func keyAlreadyExists(_ key:String) -> Bool {
    ...
    func getCaughtKeys() -> [String] {
        //print("[PlistManager] getCaughtKeys called")
        if(caughtKeys.count == 0) {
            if let plist = Plist(name: plistFileName) {
                let dict = plist.getMutablePlistFile()!
                caughtKeys = Array(dict.allKeys) as! [String]
                //print("[PlistManager] Keys are: \(keys)")
            }
        
            for i in (0...caughtKeys.count-1).reversed() {
                let currKey:String = caughtKeys[i]
                if( currKey == "Dinosaurs" || currKey == "") {
                    caughtKeys.remove(at: i)
                } else {
                    if let found = getValueForKey(currKey.lowercased()) {
                        let foundDict = found as! NSDictionary
                        //print("[PlistManager] found after getv4key")
